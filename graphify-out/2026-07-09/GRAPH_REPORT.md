...
- [[_COMMUNITY_campaign.service.js|campaign.service.js]]
...
- [[_COMMUNITY_claim.service.js|claim.service.js]]
...
- `loginUser()` --calls--> `getProfile()`  [EXTRACTED]
  src/services/auth.service.ts → src/services/user.service.ts
- `createPayment()` --calls--> `calculateTotalPayment()`  [EXTRACTED]
  src/services/payment.service.ts → src/services/wallet.service.ts
...
Cohesion: 0.14
Nodes (14): Appwrite Functions (Server-side), Authentication — API, `forgotPassword(email, redirectUrl?)` — [Client SDK], `getCurrentUser()` — [Client SDK], Lihat Juga, `loginUser(email, password, role?)` — [Client SDK], `loginWithGoogle(role?)` — [Client SDK via Appwrite SDK], `logoutUser()` — [Client SDK] (+6 more)
...
Cohesion: 0.20
Nodes (10): Balance vs Pending Balance, Campaign Top-Up (Buyer Side), Escrow, Minimum Campaign Budget, Payment Gateway, Payments — Business Rules, Platform Fee, Rate Card Order (Seller Side) (+2 more)
...
Cohesion: 0.11
Nodes (16): Field Wajib Paket, Kepemilikan, Platform Fee, Rate Cards — Business Rules, Status Rate Card, Appwrite Functions (Server-side), `createRateCard()` — [Client SDK], Discovery Creator (+8 more)
...
Cohesion: 0.25
Nodes (8): Appwrite Functions, Aturan Backend, create-user-profile, Cross-Module Dependencies, delete-file, Lihat Juga, Users — Backend, validate-and-upload
...
Cohesion: 0.19
Nodes (12): account, appwriteClient, COLLECTIONS, FUNCTIONS, databases, functions, Claim, claimCampaign() (+4 more)
...
Cohesion: 0.33
Nodes (6): ADR-008 — Platform Fee 5% (Per-Modul), Consequences, Context, Decision, Referensi, Status
